...
const CategoriesPage = () => {
  ...
  const [editModal, setEditModal] = useState({ open: false, form: defaultForm });

  useEffect(() => {
    if (!createForm.type && types.length) {
      setCreateForm((prev) => ({ ...prev, type: prev.type || types[0].slug }));
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!createForm.name.trim() || !createForm.type) return;
    try {
      await saveCategory({ ...createForm, name: createForm.name.trim() });
      setCreateForm((prev) => ({ ...defaultForm, type: prev.type || types[0]?.slug || '' }));
    } catch (err) {
      window.alert(err.message || 'Unable to save category');
    }
  };

  const openEditModal = (category) => setEditModal({ open: true, form: { ...category, type: category.type } });
  const closeEditModal = () => setEditModal({ open: false, form: defaultForm });
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      await saveCategory({ ...editModal.form, name: editModal.form.name.trim() });
      closeEditModal();
    } catch (err) {
      window.alert(err.message || 'Unable to save category');
    }
  ...
  const handleDelete = async (category) => {
    ...
    try {
      ...
      if (editModal.form.id === category.id) {
        closeEditModal();
      }
    } catch (err) {
          ...
          return (
            <div key={item.id} className="rounded-3xl border border-slate-100 bg-white/70 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
              ...
              <div className="mt-4 flex gap-2">
                <button onClick={() => openEditModal(item)} className="rounded-xl border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-600 hover:border-brand-300 dark:border-slate-700 dark:text-slate-200">
                  {t('actions.edit')}
                </button>
                <button
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <Card title={t('categories.add')}>
        <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-3">
          <label className="space-y-1 text-sm">
            ...
            <input value={createForm.name} onChange={(event) => setCreateForm((prev) => ({ ...prev, name: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm focus:border-brand-500 focus:outline-none" required />
          </label>
          <label className="space-y-1 text-sm">
            <span className="text-slate-500">{t('transactions.type')}</span>
            {types.length ? (
              <select value={createForm.type} onChange={(event) => setCreateForm((prev) => ({ ...prev, type: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700">
                {types.map((type) => {
          ...
          <label className="space-y-1 text-sm">
            ...
            <input type="color" value={createForm.color} onChange={(event) => setCreateForm((prev) => ({ ...prev, color: event.target.value }))} className="h-12 w-full cursor-pointer rounded-2xl border border-slate-200" />
          </label>
      ...
      <Modal open={editModal.open} onClose={closeEditModal} title={t('actions.edit')}>
        <form onSubmit={handleEditSubmit} className="space-y-4">
          <label className="space-y-1 text-sm">
            <span className="text-slate-500">{t('forms.name')}</span>
            <input name="name" value={editModal.form.name || ''} onChange={handleEditChange} className="w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm focus:border-brand-500 focus:outline-none" required />
          </label>
          <label className="space-y-1 text-sm">
            <span className="text-slate-500">{t('transactions.type')}</span>
            {types.length ? (
              <select name="type" value={editModal.form.type || ''} onChange={handleEditChange} className="w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700">
                {types.map((type) => {
                  const label = resolveTypeLabel(type, t) || type.name;
                  return (
                    <option key={type.id} value={type.slug}>
          ...
          <label className="space-y-1 text-sm">
            <span className="text-slate-500">{t('forms.color')}</span>
            <input name="color" type="color" value={editModal.form.color || '#5c7cfa'} onChange={handleEditChange} className="h-12 w-full cursor-pointer rounded-2xl border border-slate-200" />
          </label>
          <div className="flex flex-col gap-2 sm:flex-row">
            <button type="button" onClick={closeEditModal} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 dark:border-slate-700 dark:text-slate-300">
              {t('actions.cancel')}
            </button>
